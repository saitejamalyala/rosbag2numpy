...
class dataset_loader:
    ...
    def __init__(
        self, tfrec_dir: Path, train_size: float = 0.7, batch_size: int = 16, shuffle_buffer: int=32*100,
    ...
    def __prepare_sample(self, example):
        ...
        if self.normalize_coords==True:
            ...
            example["init_path"] = example["init_path"]/nf
            example["opt_path"] = example["opt_path"]/nf
            example["car_odo"] = example["car_odo"]/nf
            example["diff_path"] = example["diff_path"]/nf

        if self.data_set_name == 'test_all_data':
            return (example["feature_vector"],example["cost_map"],example["init_path"],example["opt_path"],example["car_odo"],
                example["file_details"]),example["diff_path"]

        return (example["feature_vector"]),example["diff_path"]
    ...
    def get_dataset(self, filenames: List[Path], batch_size: int, data_set_name: str):
        ...
        """
        try:
            assert data_set_name in ("train", "valid", "test","test_all_data")
    ...
    def build_scenario_dataset(self,no_train_scene:int,no_valid_scene:int,no_test_scene:int):
        ...
        try:
            ...
            print(f"Test scenario:{list_test}")
            """
            ...
            ds_test_alldata = self.get_dataset(
                test_tfrecs, batch_size=self.batch_size, data_set_name="test_all_data"
            )

            print(f"Number of records, Train files:{len(train_tfrecs)}, validation files:{len(valid_tfrecs)}, Test Files:{len(test_tfrecs)} \n")

            print("train,valid,test:",ds_train.element_spec)
            #pprint.pprint(ds_test.element_spec, width=1)
            print("Test set with all data:", ds_test_alldata.element_spec)

            #glob(f"{self.tfrecords_dir}/**/*.tfrec", recursive=True)
        except AssertionError as Err:
            print(f"Build scenario dataset failed, {Err}")

        return ds_train, ds_valid, ds_test, ds_test_alldata
